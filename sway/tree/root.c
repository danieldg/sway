...
struct sway_root *root_create(void) {
	...
	root->staging = alloc_scene_node(&root_scene->node, &alloc_failure);

	size_t num_layers = sizeof(root->layers) / sizeof(struct wlr_scene_node *);
	for (size_t i = 0; i < num_layers; i++) {
		((struct wlr_scene_node **) &root->layers)[i] =
			alloc_scene_node(&root_scene->node, &alloc_failure);
	}

	scene_descriptor_assign(root->layers.seat,
		SWAY_SCENE_DESC_NON_INTERACTIVE, NULL);
	if (!root->layers.seat->data) {
		alloc_failure = true;
	}

	if (alloc_failure) {
		wlr_scene_node_destroy(&root_scene->node);
		free(root);
		return NULL;
	}

	wlr_scene_node_set_enabled(root->staging, false);
...
void root_destroy(struct sway_root *root) {
	...
	wlr_scene_node_destroy(&root->root_scene->node);
