...
struct sway_workspace *workspace_create(struct sway_output *output,
	...
	ws->layers.tiling = alloc_scene_node(root->staging, &alloc_failure);
	ws->layers.fullscreen = alloc_scene_node(root->staging, &alloc_failure);

	if (alloc_failure) {
		wlr_scene_node_destroy(ws->layers.tiling);
		wlr_scene_node_destroy(ws->layers.fullscreen);
		free(ws);
		return NULL;
	}
...
void workspace_destroy(struct sway_workspace *workspace) {
	...
	wlr_scene_node_destroy(workspace->layers.tiling);
	wlr_scene_node_destroy(workspace->layers.fullscreen);
