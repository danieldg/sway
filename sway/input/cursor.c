...
struct sway_node *node_at_coords(
	...
	for (int i = num_layers - 1; i >= 0 && !scene_node; i--) {
		struct wlr_scene_node *layer = ((struct wlr_scene_node **) &root->layers)[i];
		struct sway_scene_descriptor *desc = layer->data;

		if (!desc || desc->type != SWAY_SCENE_DESC_NON_INTERACTIVE) {
			scene_node = wlr_scene_node_at(layer, lx, ly, sx, sy);
		}
	}

	if (scene_node) {
		// determine what wlr_surface we clicked on
		if (scene_node->type == WLR_SCENE_NODE_SURFACE) {
			*_surface = wlr_scene_surface_from_node(scene_node)->surface;
		}
...
static void handle_touch_motion(struct wl_listener *listener, void *data) {
	...
	if (seat->touch_id == event->touch_id) {
		...
		wl_list_for_each(node, &seat->drag_icons->state.children, state.link) {
